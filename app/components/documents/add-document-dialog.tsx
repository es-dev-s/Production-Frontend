    ...
    while (active.current < TITLE_CONCURRENCY && queue.current.length > 0) {
      ...
      void suggestTitle(file, ac.signal)
        .then((data) => {
          ...
          if (titleReady(value) || (tries.current.get(key) || 0) >= TITLE_MAX_TRIES) {
            settle(value);
            return;
          }
          retry();
        ...
        .catch((error) => {
          ...
          if (
            shouldRetryTitle(error) &&
            (tries.current.get(key) || 0) < TITLE_MAX_TRIES
          ) {
            retry();
            return;
          }
          setTitles((prev) =>
            prev[key] ? { ...prev, [key]: { pending: false, value: prev[key].value || "" } } : prev,
          );
        })
        .finally(() => {
          if (!retrying) finish();
    ...
    while (active.current < TITLE_CONCURRENCY && queue.current.length > 0) {
      ...
      void inspectWithRetry(file, ac.signal)
        .then((data) => {
          ...
          const server =
            data.uniqueness === "duplicate" ? "duplicate" : "unique";
          setInspect((prev) =>
            applyIntra({
              ...prev,
              [key]: {
        ...
        .catch((error) => {
          // A timeout abort is not the dialog closing. Title uploads can hold
          // every browser connection, so this check must retry once they finish.
          if (ac.signal.aborted) return;
          if (isAbortError(error) || shouldRetryTitle(error)) {
            retry();
            return;
          }
          giveUp();
          ...
          <div className="mt-5">
            ...
            {hasDuplicate ? (
              <p className="mt-1.5 text-[11px] leading-4 text-muted-soft">
                {admin
                  ? "A file matches an existing document. A note is optional."
                  : "A file matches an existing document. Request review to send it on. A note is optional."}
              </p>
            ) : files.length === 0 ? (
              <p className="mt-1.5 text-[11px] leading-4 text-muted-soft">
