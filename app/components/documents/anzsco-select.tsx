...
import { ANZSCO_OCCUPATIONS, findAnzsco } from "@/app/lib/anzsco";
...
export function AnzscoSelect({ value, onChange }: Props) {
  const labelId = useId();
  const listId = useId();
  ...
  const selected = findAnzsco(value);
  ...
  const options = useMemo(() => {
    ...
    if (!q) return ANZSCO_OCCUPATIONS;
    return ANZSCO_OCCUPATIONS.filter(
      (item) =>
  ...
  const place = () => {
    ...
    const width = Math.max(rect.width, 22 * 16);
