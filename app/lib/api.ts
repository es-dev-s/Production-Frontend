...
// A single 401 can be a blip (proxy abort, database retry). Only leave the
// app when the session cookie itself no longer authenticates.
async function sessionStillValid(): Promise<boolean> {
  if (!sessionCheck) {
    sessionCheck = fetch(url("/v1/auth/me"), {
      cache: "no-store",
      credentials: "include",
    })
...
async function loseSessionIfNeeded(response: Response, path: string) {
  ...
  if (await sessionStillValid()) return;
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    await loseSessionIfNeeded(response, path);
